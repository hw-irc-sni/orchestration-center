...
`etc/conf/a2at.env` is generated by `common/a2at_config.py` from the resolved `chat` capability of `common/config/llm_config.json` and rewritten on every startup — never edit it, and never put `A2AT_*` variables in the repo-root `.env` (they are ignored).

No LLM provider is hardcoded. Any scalar field of any capability can be overridden with `LLM_<CAPABILITY>_<FIELD>` (e.g. `LLM_CHAT_PROVIDER`, `LLM_CHAT_MODEL`, `LLM_CHAT_API_KEY`, `LLM_CHAT_URL`, `LLM_CHAT_BASE_URL`, `LLM_EMBED_URL`), resolved once in `_ModelConfigHolder._load()` via `common/llm/config/env_overrides.py`. Precedence: environment > repo-root `.env` > JSON. Structured fields (`auth`, `headers`, `body`, `response`) are request templates and stay JSON-only.
